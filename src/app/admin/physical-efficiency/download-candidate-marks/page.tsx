...
import {
  ...
  CallGetPhysicalWorkScopeByVendorId,
  CallPhysicalDownloadCandidateMarks,
} from "@/_ServerActions";
...
const DownloadCandidateMarks = (props: Props) => {
  ...
  const getVendorByAdvId = async (advertisementId: string) => {
    ...
    try {
      ...
      const { data, error } = (await CallGetPhysicalByAdvertisement(
        query,
      )) as any;
  ...
  const getAllWorkScope = async (verdorId: string) => {
    ...
    try {
      ...
      const { data, error } = (await CallGetPhysicalWorkScopeByVendorId(
        query,
      )) as any;
  ...
  const downloadExamMarks = async () => {
    try {
      ...
      const { data, error } = (await CallPhysicalDownloadCandidateMarks(
        submitData,
      )) as any;
