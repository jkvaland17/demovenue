...
import {
  ...
  CallPhysicalTestUpload,
  CallSavePhysicalSelection,
  CallUploadDVPSTData,
...
const VendorSelection = (props: Props) => {
  ...
  const renderCell = React.useCallback((item: any, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "addedOn":
        return <p>{moment(item?.createdAt).format("DD-MM-YYYY")}</p>;
      case "actions":
        return (
  ...
  async function emailSent(id: string, saveAdvertisementId: string) {
    const query = `_id=${id}&type=physical_efficiency`;
  ...
  const getVendorById = async (advertisementId: string, filter: boolean) => {
    ...
    try {
      ...
      const { data, error } = (await CallGetPhysicalById(
  ...
  const postVendorSelection = async () => {
    ...
    try {
      ...
      const { data, error } = (await CallSavePhysicalSelection(
        submitData,
      )) as any;
  ...
  const Upload = async (file: any) => {
    try {
      ...
      const { data, error, func } = (await CallPhysicalTestUpload(
        formData,
      )) as any;
      console.log("CallPhysicalTestUpload", { data, error });
      if (data) {
        toast.success(data?.message);
        getVendorById(saveAdvertisementId, false);
        onCloseUpload();
  ...
  return (
    <>
      <FlatCard heading="Physical Efficiency Selection">
        <div className="grid grid-cols-2 items-end gap-4 mob:grid-cols-2">
          <Select
            items={allCourses ?? []}
            label="Course"
        ...
        topContent={
          <>
            <h2 className="text-xl font-semibold">
              Selected Physical Efficiency Table
            </h2>
            ...
            <>
              ...
              <ModalBody className="gap-6">
                <CustomMultipleUpload
                  ...
                  sampleDownload={true}
                  sampleExcelUrl="/file/Physical-Efficiency.xlsx"
                  preview={preview}
