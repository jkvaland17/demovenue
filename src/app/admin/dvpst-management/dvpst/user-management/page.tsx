...
const UserManagement = (props: Props) => {
  ...
  const createUser = async (userData: object) => {
    try {
      ...
      const { data, error } = (await CallCreateDvpst(payload)) as any;
      if (data) {
        console.log(data);
      }
  ...
  const updateUser = async (userData: any) => {
    ...
    try {
      const { data, error } = (await CallUpdateDvpstMember(
        structuredData,
      )) as any;
  ...
  const getAllUsers = async (isFilter: boolean) => {
    setIsLoading(true);
    try {
      const filterOn = `page=${page}&limit=10&name=${filterData?.name}&email=${filterData?.email}&phone=${filterData?.phone}&advertisementId=${currentAdvertisementID}`;
      const filterOff = `page=${page}&limit=10&advertisementId=${currentAdvertisementID}`;
      const { data, error } = (await CallGetDvpstAllMembers(
        isFilter ? filterOn : filterOff,
      )) as any;
      console.log("CallGetDvpstAllMembers", { data, error });
      if (data) {
      ...
      <Table
        ...
        topContent={
          <>
            ...
            <div className="grid grid-cols-1 items-center gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              <Input
                placeholder="Name"
