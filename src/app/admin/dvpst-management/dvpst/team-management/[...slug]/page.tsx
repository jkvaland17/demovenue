...
import DvpstAdvertisement from "@/components/dvpst/DvpstAdvertisement";
import DvpstMemberTable from "@/components/DvpstMemberTable";
...
const ViewEditDepartment = () => {
  ...
  const [allMandal, setAllMandal] = useState<any[]>([]);
  ...
  console.log({ zoneId, mandalId, districtId });
  ...
  const getAllData = async (isFilter: boolean): Promise<void> => {
    try {
      ...
      console.log("CallGetAllMembers", { data, error });
      if (data?.message) {
  ...
  const submitData = async () => {
    try {
      ...
      const response = await CallUpdateDvpstGroup(data);
      if (response?.data) {
  ...
  const getAllMandal = async (zoneId: string) => {
    try {
      setIsLoading((prev: any) => ({
        ...prev,
        mandals: true,
      ...
      const { data, error } = (await CallGetDvpstMandal(query)) as any;
      console.log("CallGetDvpstMandal", { data, error });
      if (data.status_code === 200) {
        // getAllDistricts(zoneId);
        setAllMandal(data?.data);
        setIsLoading((prev: any) => ({
          ...prev,
          mandals: false,
        }));
  ...
  const getAllDistricts = async (zoneId: string) => {
    try {
      setIsLoading((prev: any) => ({
        ...prev,
        districts: true,
      ...
      const query = `zoneId=${zoneId}`;
      const { data, error } = (await CallGetAllDvpstDistricts(query)) as any;
      ...
      <Card className="max-w-full p-2">
        <CardHeader className="flex gap-3">
          <div className="flex w-full items-center justify-between gap-x-3 text-2xl">
            ...
            <Button className="mob:hidden" onPress={() => route.back()} radius="full">
        ...
        <CardBody>
          ...
          <div className="mt-4">
            <Select
              ...
              radius="sm"
              isRequired
              selectedKeys={[mandalId]}
              isLoading={isLoading?.mandals}
              onSelectionChange={(e: any) => {
                const value = Array.from(e)[0] as string;
                setMandalId(value);
                getAllMandal(value);
