...
import {
  CallGetUsersByCenter,
  CallGetUsersByShiftWise,
} from "@/_ServerActions";
...
const CenterUsers = (props: Props) => {
  ...
  const getUsersByShiftWise = async () => {
    setIsLoading(true);
    try {
      const query = `centerId=${id}`;
      const { data, error } = (await CallGetUsersByShiftWise(query)) as any;
      console.log("CallGetUsersByShiftWise", { data, error });
