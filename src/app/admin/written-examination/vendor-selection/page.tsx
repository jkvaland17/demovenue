...
import {
  ...
  CallGetVendorById,
  CallSaveVendorSelection,
  CallUploadDVPSTData,
  CallVendorCredential,
  CallWrittenExamUpload,
} from "@/_ServerActions";
...
const VendorSelection = (props: Props) => {
  ...
  const renderCell = React.useCallback((item: any, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "addedOn":
        3;
        return <p>{item?.createdAt ? moment(item?.createdAt).format("DD-MM-YYYY") : "-"}</p>;
      case "actions":
        return (
  ...
  const getAllVendor = async (advertisementId: string) => {
    try {
      ...
      console.log("getAllVendor", { data, error });
  ...
  const getVendorById = async (advertisementId: string, filter: boolean) => {
    ...
    try {
      ...
      const { data, error } = (await CallGetVendorById(
  ...
  async function emailSent(id: string, saveAdvertisementId: string) {
    const query = `_id=${id}&type=written_examination`;
  ...
  const postVendorSelection = async () => {
    ...
    try {
      ...
      const { data, error } = (await CallSaveVendorSelection(
        submitData,
      )) as any;
  ...
  const Upload = async (file: any) => {
    try {
      ...
      const { data, error, func } = (await CallWrittenExamUpload(
        formData,
      )) as any;
      console.log("CallWrittenExamUpload", { data, error });
      if (data) {
        toast.success(data?.message);
        getVendorById(saveAdvertisementId, false);
        onCloseUpload();
  ...
  return (
    <>
      <FlatCard heading="Vendor Selection">
        <div className="grid grid-cols-2 flex-col gap-4 mob:flex">
          <Select
            items={allCourses}
            label="Course"
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              console.log("recrutment", e.target.value);
      ...
      <Table
        ...
        topContent={
          <>
            <h2 className="text-xl font-semibold">Selected Vendor Table</h2>
            <div className="flex gap-5">
              ...
              {/* <ExcelPdfDownload
                excelFunction={() => {
                  DownloadKushalExcel(``, "Written Examination", setLoader);
                }}
                pdfFunction={() => {
                  DownloadKushalPdf(``, "Written Examination", setLoader);
                }}
            ...
            <>
              ...
              <ModalBody className="gap-6">
                <CustomMultipleUpload
                  ...
                  sampleDownload={true}
                  sampleExcelUrl="/file/Written-Examination.xlsx"
                  preview={preview}
