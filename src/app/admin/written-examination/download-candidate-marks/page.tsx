"use client";
import {
  CallDownloadCandidateMarks,
  CallGetAdvByCourse,
  CallGetAllCourses,
  CallGetAllWorkScope,
  CallGetVendorById,
  CallGetWorkScopeByVendorId,
} from "@/_ServerActions";
...
const DownloadCandidateMarks = (props: Props) => {
  ...
  const getAllWorkScope = async (verdorId: string) => {
    ...
    try {
      ...
      const { data, error } = (await CallGetWorkScopeByVendorId(query)) as any;
  ...
  const downloadExamMarks = async () => {
    try {
      ...
      const { data, error } = (await CallDownloadCandidateMarks(
        submitData,
      )) as any;
      console.log("downloadExamMarks", { data, error });
