...
import {
  ...
  CallGetAllCourses,
  CallGetVendorById,
  CallGetVendorByIdUpload,
  CallUploadCandidateExam,
} from "@/_ServerActions";
...
const UploadCandidateExamData = (props: Props) => {
  ...
  const getVendorByAdvId = async (advertisementId: string) => {
    ...
    try {
      ...
      // const { data, error } = (await CallGetVendorById(query)) as any;
      const { data, error } = (await CallGetVendorByIdUpload(query)) as any;
  ...
  const uploadCandidateExam = async () => {
    ...
    try {
      const { data, error } = (await CallUploadCandidateExam(formData)) as any;
  ...
  const downloadTemplate = async () => {
    try {
      setLoader(true);
      const { data, error } = (await CallDownloadTemplate()) as any;
      console.log("CallDownloadTemplate", data, error);
      if (data?.fileUrl) {
        toast.success(data?.message);
        setLoader(false);
  ...
  const downloadDataByBoard = async () => {
    try {
      ...
      const { data, error } = (await CallDownloadDataByBoard(payload)) as any;
      console.log("CallDownloadDataByBoard", { data, error });
      if (data?.fileUrl) {
        toast.success(data?.message);
