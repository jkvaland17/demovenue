...
import {
  CallGetAdvByCourse,
  CallGetAllCourses,
  CallGetFinalResultByAdvertisement,
  CallFinalResultUploadCandidateExam,
  CallFileCheck,
  CallGetFinalResultByAdvertisementUpload,
  CallDownloadTemplate,
  CallDownloadTemplateFinal,
  CallDownloadDataByBoardTypingTest,
  CallDownloadDataByBoardFinalResult,
} from "@/_ServerActions";
...
const UploadCandidateExamData = (props: Props) => {
  ...
  const getVendorByAdvId = async (advertisementId: string) => {
    ...
    try {
      ...
      // const { data, error } = (await CallGetFinalResultByAdvertisement(query,)) as any;
      const { data, error } = (await CallGetFinalResultByAdvertisementUpload(
        query,
      )) as any;
  ...
  const uploadCandidateExam = async () => {
    ...
    try {
      const { data, error } = (await CallFinalResultUploadCandidateExam(
        formData,
      )) as any;
  ...
  const downloadTemplate = async () => {
    try {
      setLoader(true);
      const { data, error } = (await CallDownloadTemplateFinal()) as any;
      console.log("CallDownloadTemplateFinal", data, error);
      if (data?.fileUrl) {
        toast.success(data?.message);
        setLoader(false);
  ...
  const downloadDataByBoard = async () => {
    try {
      ...
      const { data, error } = (await CallDownloadDataByBoardFinalResult(
        payload,
      )) as any;
      console.log("CallDownloadDataByBoardFinalResult", data, error);
      if (data?.fileUrl) {
        toast.success(data?.message);
