"use client";
import {
  CallDownloadDataByBoard,
  CallDownloadDataByBoardTypingTest,
  CallDownloadTemplate,
  CallDownloadTemplateTypingTest,
  CallFileCheck,
  CallGetAdvByCourse,
  CallGetAllCourses,
  CallGetTypingByAdvertisement,
  CallGetTypingByAdvertisementUpload,
  CallTypingUploadCandidateExam,
} from "@/_ServerActions";
...
const UploadCandidateExamData = (props: Props) => {
  ...
  const getVendorByAdvId = async (advertisementId: string) => {
    ...
    try {
      ...
      // const { data, error } = (await CallGetTypingByAdvertisement(query)) as any;
      const { data, error } = (await CallGetTypingByAdvertisementUpload(
        query,
      )) as any;
  ...
  const uploadCandidateExam = async () => {
    ...
    try {
      const { data, error } = (await CallTypingUploadCandidateExam(
        formData,
      )) as any;
  ...
  const downloadTemplate = async () => {
    try {
      setLoader(true);
      const { data, error } = (await CallDownloadTemplateTypingTest()) as any;
      console.log("CallDownloadTemplateTypingTest", data, error);
      if (data?.fileUrl) {
        toast.success(data?.message);
        setLoader(false);
  ...
  const downloadDataByBoard = async () => {
    try {
      ...
      const { data, error } = (await CallDownloadDataByBoardTypingTest(
        payload,
      )) as any;
      console.log("CallDownloadDataByBoardTypingTest", data, error);
      if (data?.fileUrl) {
        toast.success(data?.message);
