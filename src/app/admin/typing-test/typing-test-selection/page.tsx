...
import {
  ...
  CallSaveTypingSelection,
  CallTypingTestUpload,
  CallUploadDVPSTData,
...
const VendorSelection = (props: Props) => {
  ...
  async function emailSent(id: string, saveAdvertisementId: string) {
    const query = `_id=${id}&type=typing_test`;
  ...
  const getVendorById = async (advertisementId: string, filter: boolean) => {
    ...
    try {
      ...
      const { data, error } = (await CallGetTypingById(
  ...
  const postVendorSelection = async () => {
    ...
    try {
      ...
      const { data, error } = (await CallSaveTypingSelection(
        submitData,
      )) as any;
  ...
  const Upload = async (file: any) => {
    try {
      ...
      const { data, error, func } = (await CallTypingTestUpload(
        formData,
      )) as any;
      console.log("CallTypingTestUpload", { data, error });
      if (data) {
        toast.success(data?.message);
        getVendorById(saveAdvertisementId, false);
        onCloseUpload();
  ...
  return (
    <>
      <FlatCard heading="Typing Test Selection">
            ...
            <>
              ...
              <ModalBody className="gap-6">
                <CustomMultipleUpload
                  ...
                  sampleDownload={true}
                  sampleExcelUrl="/file/Typing-Test.xlsx"
                  preview={preview}
