...
import { ApplicationFeesProps } from "@/Utils/Advertisement/types";

const ApplicationFees: React.FC<ApplicationFeesProps> = ({ formMethods }) => {
  const { control, register, watch, setValue } = formMethods;

  const {
    fields: applicationFeesFields,
    append: appendApplicationFees,
    remove: removeApplicationFees,
  } = useFieldArray({
    control,
      ...
      {applicationFeesFields?.map((field, index) => (
        <div key={field.id} className="overflow-hidden rounded-lg">
          <div className="flex bg-white">
            ...
            <div className="flex-1 px-4 py-3">
              <Controller
                name={`applicationFees.${index}.amount`}
                control={control}
                render={({
                  field: { onChange, value },
                  fieldState: { error, invalid },
            ...
            <div className="flex flex-1 items-center justify-center px-4 py-3">
              {applicationFeesFields?.length > 0 && (
                <Button
                  ...
                  variant="flat"
                  size="sm"
                  onPress={() => removeApplicationFees(index)}
                >
                  Remove
                </Button>
              )}
