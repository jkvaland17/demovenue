...
const AgeRelaxationData: React.FC<Props> = ({ formMethods, postDataIndex }) => {
  ...
  const {
    ...
    control,
    name: `postData.${postDataIndex}.ageRelaxation`,
  });
  return (
    <div className="mt-5 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-lg font-medium text-gray-800">Age Relaxation</h3>
        <Button
          color="primary"
          variant="flat"
          size="sm"
        ...
        <div key={field.id} className="overflow-hidden rounded-lg">
          <div className="flex bg-white">
            <div className="flex-1 px-4 py-3">
              <Controller
                name={`postData.${postDataIndex}.ageRelaxation.${index}.category`}
                control={control}
                rules={{
            ...
            <div className="flex-1 px-4 py-3">
              <Controller
                name={`postData.${postDataIndex}.ageRelaxation.${index}.gender`}
                control={control}
                rules={{
            ...
            <div className="flex-1 px-4 py-3">
              <Controller
                name={`postData.${postDataIndex}.ageRelaxation.${index}.age`}
                control={control}
                render={({
                  field: { onChange, value },
                  fieldState: { error, invalid },
